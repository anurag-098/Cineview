...
async function showceleb(celeb) {
    ...
    for (let i = 0; i < celeb.knownFor.length; i++) {
        list += `  <div id="bcard">
        <a href="https://cineview-alpha.vercel.app/movie?imdbId=${celeb.knownFor[i].id}">
    ...
    for (let i = 0; i < celeb.moremovies.length; i++) {
        if (celeb.moremovies[i].role === "Actor") {
            morelist += `
        <a href="https://cineview-alpha.vercel.app/movie?imdbId=${celeb.moremovies[i].id}" style="text-decoration:none">
